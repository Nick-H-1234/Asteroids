public class Asteroid extends Polygon {
    ...
    public Point[] scale(Point[] shape, double multiplyer) {
        getTransformedPoints();
        Point[] scaledShape = new Point[shape.length];
        for (int i = 0; i < shape.length; i++) {
            int newx = (int) (getTransformedPoints()[i].x * multiplyer);
            int newy = (int) (getTransformedPoints()[i].y * multiplyer);
            Point point = new Point(newx, newy);
            scaledShape[i] = new Point(newx, newy);
        }
        return scaledShape;
    }
}
